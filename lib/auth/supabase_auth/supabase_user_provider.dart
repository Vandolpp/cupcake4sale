...
class Cupcake4saleSupabaseUser extends BaseAuthUser {
  Cupcake4saleSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> cupcake4saleSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = Cupcake4saleSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
